...
### Suite rapide (`tests/`, 278 tests, sans TensorFlow)
...
- Remplacement des icônes restantes par une bibliothèque unique — fait pour la navigation (Material Symbols) et les icônes SVG de `app/components.py`/`app/icons.py`, mais des emoji subsistent encore dans plusieurs pages (Historique, Rapports, Analyse, Assistant : boutons, messages d'erreur/avertissement, légendes d'image).
...
- Traduction complète des pages restantes (Analyse, Dashboard, Historique, Assistant) — seuls la navigation, À propos et Paramètres sont traduits. Les rapports PDF/JSON, eux, sont déjà multilingues (voir plus haut) : seule l'interface qui les entoure reste en français sur ces pages.
- PDF en arabe : nécessiterait d'embarquer une police arabe (Noto Sans Arabic) et les bibliothèques `arabic-reshaper`/`python-bidi` dans ReportLab. Le JSON, lui, est déjà pleinement en arabe.
